#!/usr/bin/env python3
"""
Benchmark v3 — uses compliance_scorer_v5.py (new prompt).

Saves results to benchmark_results_v5_<model>.json.
...
from compliance_scorer_v5 import score_documents, PROMPT_VERSION
...
def main():
    ...
    out_path = BASE_DIR / f"benchmark_results_v5_{safe_model}.json"
    out_path.write_text(json.dumps(results, indent=2, ensure_ascii=False))
    print(f"\nFull results saved to {out_path}")

    # ── Quick diff vs v2/v3/v4/v5 ────────────────────────────────────────────
    v2_path = BASE_DIR / f"benchmark_results_v2_{safe_model}.json"
    v3_path = BASE_DIR / f"benchmark_results_v3_{safe_model}.json"
    v4_path = BASE_DIR / f"benchmark_results_v4_{safe_model}.json"
    if v4_path.exists():
        ...
        v4 = {r["test_case"]: r for r in json.loads(v4_path.read_text())}
        print(f"\n{'='*70}")
        print(f"DIFF — v2 / v3 / v4 / v5 comparison")
        print(f"{'='*70}")
        print(f"{'Test Case':<26}  {'v2':>10}  {'v3':>10}  {'v4':>10}  {'v5':>10}  v4→v5")
        print("-" * 88)
        v2_pass = v3_pass = v4_pass = v5_pass = 0
        for r in results:
            tc = r["test_case"]
            ...
            ok4 = "✅" if (r4["success_present"] and not r4["failure_present"]) else "❌"
            ok5 = "✅" if (r["success_present"] and not r["failure_present"]) else "❌"
            ...
            if r4["success_present"] and not r4["failure_present"]: v4_pass += 1
            if r["success_present"] and not r["failure_present"]: v5_pass += 1
            s2 = (f"{r2['success_score']:.0f}/{r2['failure_score']:.0f}") if r2 else "─/─"
            s3 = (f"{r3['success_score']:.0f}/{r3['failure_score']:.0f}") if r3 else "─/─"
            s4 = f"{r4['success_score']:.0f}/{r4['failure_score']:.0f}"
            s5 = f"{r['success_score']:.0f}/{r['failure_score']:.0f}"
            print(f"{tc:<26}  {ok2} {s2:>8}  {ok3} {s3:>8}  {ok4} {s4:>8}  {ok5} {s5:>8}  {ok4}→{ok5}")
        print("-" * 88)
        print(f"{'PASS COUNT':<26}  {v2_pass}/6{'':<8}  {v3_pass}/6{'':<8}  {v4_pass}/6{'':<8}  {v5_pass}/6")
    else:
        print(f"\n(No v4 results found at {v4_path} — run benchmark_v4.py first to enable diff)")
