...
- A policy document containing a version metadata block (Version: X.X, Approval Date, Last Review Date, Next Review Date) → IS a **version history** for that policy. A separate standalone "version history log" file is NOT required. The version metadata embedded in the document header satisfies "periodic review with version tracking" requirements — score as DIRECT_MATCH if the metadata is present and shows governance approval. **Scope of this rule:** applies ONLY to formal policy, standard, or procedure documents that directly govern security requirements. It does NOT apply to technical architecture diagrams, system design documents, security tool descriptions, or operational runbooks — those cannot substitute for a governance policy, penetration test report, or audit report, regardless of any version metadata they contain.
...
- **Example (Vulnerability Management effectiveness):** a package containing [Vulnerability Management Process document with defined KPIs, scan frequencies, severity classifications, and remediation SLAs] + [KPI/KRI report tracking vulnerability metrics and targets] = DIRECT_MATCH (evidence_strength 85+). The process document proves the framework exists and is governed; the KPI/KRI report proves it is actively measured. If an audit schedule is also present listing vulnerability management as a recurring scope, this further confirms periodic review — score DIRECT_MATCH, evidence_strength 85-95. Do NOT downgrade this package merely because the process document is not titled "review report" — a governed process with measurable KPIs IS the evidence of an effective, monitored process.
